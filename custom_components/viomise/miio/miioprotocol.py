...
class MiIOProtocol:
    ...
    def send(
        ...
        *,
        ...
        # print("%s:%s >>: %s" % (self.ip, self.port, request))
        # _LOGGER.info("%s:%s >>: %s", self.ip, self.port, request)
        # if self.debug > 1:
        # _LOGGER.debug(
        #     "send (timeout %s): %s",
        #     self._timeout,
        ...
        try:
            ...
            # print("%s:%s <<: %s" % (self.ip, self.port, m.data.value))
            # _LOGGER.info("%s:%s <<: %s", self.ip, self.port, m.data.value)
