...
from custom_components.viomise.miio.click_common import EnumType, command, format_output
from custom_components.viomise.miio.device import Device, DeviceStatus
from custom_components.viomise.miio.exceptions import DeviceException
from custom_components.viomise.miio.integrations.vacuum.roborock.vacuumcontainers import (
    ConsumableStatus,
    Timer,
    DNDStatus,
)
from custom_components.viomise.miio.utils import pretty_seconds

_LOGGER = logging.getLogger(__name__)

SUPPORTED_MODELS = ["viomi.vacuum.v7", "viomi.vacuum.v8", "viomi.vacuum.v19"]
...
def _get_rooms_from_schedules(schedules: List[str], map_id = None) -> Tuple[bool, Dict]:
    ...
    * https://github.com/homebridge-xiaomi-roborock-vacuum/homebridge-xiaomi-roborock-vacuum#semi-automatic
    ...
    schedules = json.loads(schedules["out"][0]["value"])
    for raw_schedule in schedules:
        schedule = raw_schedule.split("_")
        # Scheduled cleanup needs to be scheduled for 00:00 and inactive
        if schedule[1] == "0" and schedule[3] == "0" and schedule[4] == "0" and (map_id and schedule[10] == str(map_id)):
...
class ViomiVacuum(Device):
    """Interface for Viomi vacuums (viomi.vacuum.v19)."""
    ...
    def __init__(
        ...
        *,
        ...
        self.map_id = None
        self._cache: Dict[str, Any] = {"edge_state": None, "rooms" + str(self.map_id): {}, "maps": {}}

    def setTimeout(self, timeout):
        timeout = timeout
        super().__init__(self.ip, self.token, self.start_id, self.debug, model=self.model)
    ...
    def start_with_room(self, rooms):
        ...
        if not self._cache["rooms"+str(self.map_id)]:
            self.get_rooms()
        reverse_rooms = {v: k for k, v in self._cache["rooms"+str(self.map_id)].items()}
        room_ids = []
        for room in rooms:
            if room in self._cache["rooms"+str(self.map_id)]:
                room_ids.append(int(room))
            elif room in reverse_rooms:
                room_ids.append(int(reverse_rooms[room]))
            else:
                room_keys = ", ".join(self._cache["rooms"+str(self.map_id)].keys())
                room_ids = ", ".join(self._cache["rooms"+str(self.map_id)].values())
                raise DeviceException(
                    f"Room {room} is unknown, it must be in {room_keys} or {room_ids}"
                )

        # self._cache["edge_state"] = self.get_properties(["mode"])
        self._cache["edge_state"] = [0] # 0 - Normal, 1 - edge only
        self.send(
            "set_mode_withroom",
    ...
    def get_maps(self) -> List[Dict[str, Any]]:
        ...
        if not self._cache["maps"]:
            self._cache["maps"] = self.send("get_map")["out"][0]["value"]
        return json.loads(self._cache["maps"])
    ...
    def get_rooms(
        ...
        if self._cache["rooms"+str(self.map_id)] and not refresh:
            return self._cache["rooms"+str(self.map_id)]
        ...
        if map_name:
            ...
            if self.map_id not in [m["id"] for m in maps]:
        ...
        scheduled_found, rooms = _get_rooms_from_schedules(schedules, self.map_id)
        ...
        self._cache["rooms"+str(self.map_id)] = rooms
        return rooms
