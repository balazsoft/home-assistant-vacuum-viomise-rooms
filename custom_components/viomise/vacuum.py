...
from custom_components.viomise.miio import (
    DeviceException,
    ViomiVacuum,
)  # pylint: disable=import-error
import voluptuous as vol
from custom_components.viomise.xiaomi_cloud_map_extractor.const import CONF_COUNTRY
from homeassistant.exceptions import PlatformNotReady

from . import miio_util
...
DEFAULT_NAME = "Viomi Robot Vacuum Cleaner SE V-RVCLM21A Rooms"
...
SERVICE_SCHEMA_CLEAN_ROOM = VACUUM_SERVICE_SCHEMA.extend(
    {
        vol.Required(ATTR_ZONE_ARRAY): vol.All(
            list,
            [vol.Coerce(str)],
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    # vacuum_rooms = miio_util.ViomiSE3Rooms(host, token, username, password, country)
    vacuum_rooms = await hass.async_add_executor_job(
        miio_util.ViomiSE3Rooms, host, token, username, password, country
    )

    mirobo = MiroboVacuum2(name, vacuum, vacuum_rooms)
...
class MiroboVacuum2(StateVacuumEntity):
    ...
    def __init__(self, name, vacuum, vacuum_rooms):
        ...
        self._name = name
        self._vacuum = vacuum
        self._vacuum_rooms = vacuum_rooms
    ...
    def extra_state_attributes(self):
        ...
        if self.vacuum_state is not None:
            ...
            try:
                attrs["status"] = STATE_CODE_TO_STATE[
                    int(self.vacuum_state["run_state"])
                ]
            except KeyError:
                return (
                    "Definition missing for state %s" % self.vacuum_state["run_state"]
                )
        return attrs
    ...
    async def async_stop(self, **kwargs):
        ...
        if mode == 3:
            ...
            param = [0]
        await self._try_command(
            "Unable to stop: %s", self._vacuum.raw_command, method, param
        )
    ...
    def update(self):
        ...
        try:
            ...
            mapping2 = [{"did": "rooms", "siid": 0, "piid": 0}]
            ...
            try:
                rooms = self._vacuum_rooms.getRooms()
            except Exception as e:
                _LOGGER.error("self._vacuum_rooms.getRooms: %s", e)
                return False
            ...
            room_names = []
            for room in rooms:
                self.rooms[room[0]] = [room[1], room[2], room[3], room[4], room[5]]
                room_names.append(room[0])
            state.append(room_names)
    ...
    async def async_clean_room(self, zone, repeats=1):
        """Clean selected rooms for the number of repeats indicated."""
        ...
        if len(room_ids) == 0:
            _LOGGER.error("Invalid room name(s)")
            return False
