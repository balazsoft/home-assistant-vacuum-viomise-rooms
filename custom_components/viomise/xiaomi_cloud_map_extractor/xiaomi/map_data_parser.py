...
from custom_components.viomise.xiaomi_cloud_map_extractor.common.map_data import *
from custom_components.viomise.xiaomi_cloud_map_extractor.common.map_data_parser import (
    MapDataParser,
)
from custom_components.viomise.xiaomi_cloud_map_extractor.types import (
    Colors,
    Drawables,
    Sizes,
    Texts,
)
from custom_components.viomise.xiaomi_cloud_map_extractor.xiaomi.image_handler import (
    ImageHandlerXiaomi,
)
...
class MapDataParserXiaomi(MapDataParser):
    ...
    def parse(
        ...
        **kwargs
        ...
        while block_start_position < len(raw):
            block_header_length = MapDataParserXiaomi.get_int16(
                raw, block_start_position + 0x02
            )
            header = MapDataParserXiaomi.get_bytes(
            ...
            if block_type == MapDataParserXiaomi.CHARGER:
                ...
                map_data.vacuum_position = MapDataParserXiaomi.parse_object_position(
                    block_data_length, data
                )
            elif block_type == MapDataParserXiaomi.PATH:
                map_data.path = MapDataParserXiaomi.parse_path(
                    block_start_position, header, raw
                )
            elif block_type == MapDataParserXiaomi.GOTO_PATH:
                map_data.goto_path = MapDataParserXiaomi.parse_path(
                    block_start_position, header, raw
                )
            elif block_type == MapDataParserXiaomi.GOTO_PREDICTED_PATH:
                map_data.predicted_path = MapDataParserXiaomi.parse_path(
                    block_start_position, header, raw
                )
                ...
                map_data.ignored_obstacles = MapDataParserXiaomi.parse_obstacles(
                    data, header
                )
            elif block_type == MapDataParserXiaomi.OBSTACLES_WITH_PHOTO:
                map_data.obstacles_with_photo = MapDataParserXiaomi.parse_obstacles(
                    data, header
                )
            elif block_type == MapDataParserXiaomi.IGNORED_OBSTACLES_WITH_PHOTO:
                map_data.ignored_obstacles_with_photo = (
                    MapDataParserXiaomi.parse_obstacles(data, header)
                )
    ...
    def get_current_vacuum_room(
        ...
        room = ImageHandlerXiaomi.get_room_at_pixel(
    ...
    def parse_image(
        ...
        image, rooms_raw = ImageHandlerXiaomi.parse(
        ...
        for number, room in rooms_raw.items():
            rooms[number] = Room(
                number,
                MapDataParserXiaomi.image_to_map(room[0] + image_left),
        ...
        return (
            ImageData(
                ...
                image_config,
                image,
                MapDataParserXiaomi.map_to_image,
            ),
            rooms,
        )
    ...
    def parse_obstacles(data: bytes, header: bytes) -> List[Obstacle]:
        ...
        for obstacle_start in range(0, obstacle_pairs * obstacle_size, obstacle_size):
            ...
            details = {}
            if obstacle_size >= 6:
                details[ATTR_TYPE] = MapDataParserXiaomi.get_int16(
                    data, obstacle_start + 4
                )
                if details[ATTR_TYPE] in MapDataParserXiaomi.KNOWN_OBSTACLE_TYPES:
                    details[
                        ATTR_DESCRIPTION
                    ] = MapDataParserXiaomi.KNOWN_OBSTACLE_TYPES[details[ATTR_TYPE]]
                if obstacle_size >= 10:
                    ...
                    if obstacle_size == 28 and (data[obstacle_start + 12] & 0xFF) > 0:
                        txt = MapDataParserXiaomi.get_bytes(
