...
from custom_components.viomise.xiaomi_cloud_map_extractor.roidmi.image_handler import (
    ImageHandlerRoidmi,
)
...
class MapDataParserRoidmi(MapDataParser):
    @staticmethod
    def parse(
        raw: bytes, colors, drawables, texts, sizes, image_config, *args, **kwargs
    ) -> MapData:
        ...
        image = MapDataParserRoidmi.parse_image(
            map_image,
            width,
            height,
            x_min_calc,
        ...
        (
            ...
            map_data.walls,
        ) = MapDataParserRoidmi.parse_areas(map_info)
        if not map_data.image.is_empty:
            MapDataParserRoidmi.draw_elements(
                colors, drawables, sizes, map_data, image_config
            )
            if len(map_data.rooms) > 0 and map_data.vacuum_position is not None:
                map_data.vacuum_room = MapDataParserRoidmi.get_current_vacuum_room(
                    map_image, map_data, width
                )
    ...
    def get_current_vacuum_room(
        map_image: bytes, map_data: MapData, original_width: int
    ) -> Optional[int]:
    ...
    def image_to_map(p: Point, resolution, min_x, min_y) -> Point:
        return Point(
            (p.x + min_x) * resolution * 1000, (p.y + min_y) * resolution * 1000
        )
    ...
    def parse_image(
        ...
        image, rooms_raw = ImageHandlerRoidmi.parse(
            map_image, width, height, colors, image_config, room_numbers
        )
    ...
    def parse_vacuum_position(map_info: dict) -> Point:
        vacuum_position = MapDataParserRoidmi.parse_position(
            map_info, "robotPos", "robotPos", "robotPhi"
        )
        if vacuum_position is None:
            vacuum_position = MapDataParserRoidmi.parse_position(
                map_info, "posX", "posY", "posPhi"
            )
        return vacuum_position

    @staticmethod
    def parse_charger_position(map_info: dict) -> Point:
        return MapDataParserRoidmi.parse_position(
            map_info, "chargeHandlePos", "chargeHandlePos", "chargeHandlePhi"
        )

    @staticmethod
    def parse_position(
        map_info: dict, x_label: str, y_label: str, a_label: str
    ) -> Optional[Point]:
        position = None
    ...
    def parse_areas(map_info: dict) -> Tuple[List[Area], List[Area], List[Wall]]:
        ...
        if "area" in map_info:
            ...
            for area in areas:
                if (
                    "active" in area
                    and area["active"] == "forbid"
                    and "vertexs" in area
                    and len(area["vertexs"]) == 4
                ):
