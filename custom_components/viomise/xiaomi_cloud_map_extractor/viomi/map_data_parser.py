...
from custom_components.viomise.xiaomi_cloud_map_extractor.const import *
...
from custom_components.viomise.xiaomi_cloud_map_extractor.viomi.image_handler import (
    ImageHandlerViomi,
)
from custom_components.viomise.xiaomi_cloud_map_extractor.viomi.parsing_buffer import (
    ParsingBuffer,
)
...
class MapDataParserViomi(MapDataParser):
    ...
    def parse(
        ...
        **kwargs,
        ...
        if feature_flags & MapDataParserViomi.FEATURE_CHARGE_STATION != 0:
            ...
            map_data.charger = MapDataParserViomi.parse_position(
                buf, "pos", with_angle=True
            )
        ...
        if feature_flags & MapDataParserViomi.FEATURE_RESTRICTED_AREAS != 0:
            ...
            ) = MapDataParserViomi.parse_restricted_areas(buf)
        ...
        if map_data.rooms is not None:
            _LOGGER.debug(
                "rooms: %s", [str(room) for number, room in map_data.rooms.items()]
            )
        if not map_data.image.is_empty:
            MapDataParserViomi.draw_elements(
    ...
    def get_current_vacuum_room(
        buf: ParsingBuffer, vacuum_position: Point
    ) -> Optional[int]:
        vacuum_position_on_image = MapDataParserViomi.map_to_image(vacuum_position)
        pixel_type = buf.get_at_image(
            int(vacuum_position_on_image.y) * 800 + int(vacuum_position_on_image.x)
        )
        if (
            ImageHandlerViomi.MAP_ROOM_MIN
            <= pixel_type
            <= ImageHandlerViomi.MAP_ROOM_MAX
        ):
            return pixel_type
        elif (
            ImageHandlerViomi.MAP_SELECTED_ROOM_MIN
            <= pixel_type
            <= ImageHandlerViomi.MAP_SELECTED_ROOM_MAX
        ):
            return (
                pixel_type
                - ImageHandlerViomi.MAP_SELECTED_ROOM_MIN
    ...
    def parse_image(
        ...
        for number, room in rooms_raw.items():
            rooms[number] = Room(
                number,
                MapDataParserViomi.image_to_map(room[0] + image_left),
        ...
        return (
            ImageData(
                ...
                image_config,
                image,
                MapDataParserViomi.map_to_image,
    ...
    @staticmethod
    def parse_position(
        buf: ParsingBuffer, name: str, with_angle: bool = False
    ) -> Optional[Point]:
        x = buf.get_float32(name + ".x")
        y = buf.get_float32(name + ".y")
        if (
            x == MapDataParserViomi.POSITION_UNKNOWN
            or y == MapDataParserViomi.POSITION_UNKNOWN
        ):
            return None
        a = None
        if with_angle:
