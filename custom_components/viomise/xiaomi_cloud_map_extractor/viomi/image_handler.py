...
from custom_components.viomise.xiaomi_cloud_map_extractor.common.image_handler import (
    ImageHandler,
)
...
class ImageHandlerViomi(ImageHandler):
    ...
    def parse(
        ...
        ImageType, Dict[int, Tuple[int, int, int, int]], Set[int], Optional[ImageType]
    ]:
        rooms = {}
        cleaned_areas = set()
        ...
        if trimmed_width == 0 or trimmed_height == 0:
            return (
                ImageHandler.create_empty_map_image(colors),
                rooms,
                cleaned_areas,
                None,
            )
        ...
        if not b_rooms:
            ...
            for img_y in range(trimmed_height):
                ...
                for img_x in range(trimmed_width):
                    ...
                    if pixel_type == ImageHandlerViomi.MAP_OUTSIDE:
                        ...
                        pixels[x, y] = ImageHandler.__get_color__(
                            COLOR_NEW_DISCOVERED_AREA, colors
                        )
                    elif (
                        ...
                        if pixel_type < ImageHandlerViomi.MAP_SELECTED_ROOM_MIN:
                            ...
                            if draw_cleaned_area:
                                cleaned_areas_pixels[x, y] = ImageHandler.__get_color__(
                                    COLOR_CLEANED_AREA, colors
                                )
                        ...
                        default = ImageHandler.ROOM_COLORS[
                            room_number % len(ImageHandler.ROOM_COLORS)
                        ]
                        pixels[x, y] = ImageHandler.__get_color__(
            ...
            if (
                image_config["scale"] != 1
                and trimmed_width != 0
                and trimmed_height != 0
            ):
                image = image.resize(
