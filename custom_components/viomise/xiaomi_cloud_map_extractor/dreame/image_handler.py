...
from custom_components.viomise.xiaomi_cloud_map_extractor.common.map_data import Room
...
class ImageHandlerDreame(ImageHandler):
    ...
    @staticmethod
    def parse(
        raw_data: bytes, header, colors, image_config, map_data_type: str
    ) -> Tuple[ImageType, Dict[int, Room]]:
        ...
        trim_bottom = int(
            image_config[CONF_TRIM][CONF_BOTTOM] * header.image_height / 100
        )
        ...
        for img_y in range(trimmed_height):
            for img_x in range(trimmed_width):
                ...
                if map_data_type == "regular":
                    px = raw_data[
                        img_x + trim_left + header.image_width * (img_y + trim_bottom)
                    ]
                    segment_id = px >> 2
                    if 0 < segment_id < 62:
                        if segment_id not in rooms:
                            rooms[segment_id] = Room(
                                segment_id, room_x, room_y, room_x, room_y
                            )
                        rooms[segment_id] = Room(
                            segment_id,
                        ...
                        pixels[x, y] = ImageHandler.__get_color__(
                            f"{COLOR_ROOM_PREFIX}{segment_id}", colors, default
                        )
                    else:
        ...
        if (
            ...
            and header.image_width != 0
            and header.image_height != 0
        ):
            image = image.resize(
