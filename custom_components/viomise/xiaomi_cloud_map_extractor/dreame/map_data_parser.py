...
from custom_components.viomise.xiaomi_cloud_map_extractor.dreame.image_handler import (
    ImageHandlerDreame,
)
...
class MapDataParserDreame(MapDataParser):
    ...
    def decode_map(
        ...
        sizes,
        image_config,
        map_data_type=MapDataTypes.REGULAR,
    ) -> MapData:
        ...
        return MapDataParserDreame.parse(
            unzipped, colors, drawables, texts, sizes, image_config, map_data_type
        )
    ...
    def parse(
        ...
        **kwargs,
        ...
        if (
            ...
            image_raw = raw[
                MapDataParserDreame.HEADER_SIZE : MapDataParserDreame.HEADER_SIZE
                + header.image_width * header.image_height
            ]
            additional_data_raw = raw[
                MapDataParserDreame.HEADER_SIZE
                + header.image_width * header.image_height :
            ]
            ...
            if not map_data.image.is_empty:
                if map_data_type == MapDataParserDreame.MapDataTypes.REGULAR:
                    MapDataParserDreame.draw_elements(
    ...
    def parse_header(raw: bytes) -> Optional[MapDataHeader]:
        ...
        header.image_left = round(
            MapDataParserDreame.read_int_16_le(raw, 23) / header.image_pixel_size
        )
        header.image_top = round(
            MapDataParserDreame.read_int_16_le(raw, 25) / header.image_pixel_size
        )
    ...
    def parse_image(
        ...
        image_config,
        additional_data_json,
        map_data_type: MapDataTypes,
    ) -> Tuple[ImageData, Dict[int, Room]]:

        _LOGGER.debug(f"parse image for map {map_data_type}")
        image, image_rooms = ImageHandlerDreame.parse(
            image_raw, header, colors, image_config, map_data_type
        )
    ...
    def parse_virtual_walls(virtual_walls: list) -> List[Wall]:
        return [
            Wall(virtual_wall[0], virtual_wall[1], virtual_wall[2], virtual_wall[3])
