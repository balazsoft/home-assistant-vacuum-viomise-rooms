...
class ImageHandler:
    ...
    def draw_path(
        ...
        ImageHandler.__draw_path__(
            image, path, sizes, ImageHandler.__get_color__(COLOR_PATH, colors), scale
        )
    ...
    def draw_goto_path(
        ...
        ImageHandler.__draw_path__(
            ...
            path,
            sizes,
            ImageHandler.__get_color__(COLOR_GOTO_PATH, colors),
            scale,
        )
    ...
    def draw_predicted_path(
        ...
        ImageHandler.__draw_path__(
            ...
            path,
            sizes,
            ImageHandler.__get_color__(COLOR_PREDICTED_PATH, colors),
            scale,
        )
    ...
    def __draw_vacuum__(image: ImageData, vacuum_pos, r, outline, fill):
        ...
        ImageHandler.__draw_on_new_layer__(
            image, draw_func, 1, ImageHandler.__use_transparency__(outline, fill)
        )
    ...
    @staticmethod
    def __draw_path__(
        image: ImageData, path: Path, sizes: Sizes, color: Color, scale: float
    ):
        if len(path.path) < 1:
            return
        ...
        ImageHandler.__draw_on_new_layer__(
            image, draw_func, scale, ImageHandler.__use_transparency__(color)
        )
    ...
    def __draw_text__(
        ...
        ImageHandler.__draw_on_new_layer__(
            image, draw_func, 1, ImageHandler.__use_transparency__(color)
        )
